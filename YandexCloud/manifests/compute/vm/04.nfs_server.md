...
#cloud-config
  ...
  runcmd:
    - [ sh, -xc, "mkdir /data/nfs && chown -R nobody:nogroup /data/nfs && echo -n '/mnt/nfs *(rw,sync,crossmnt,fsid=0,no_subtree_check,wdelay,all_squash)' > /etc/exports && exportfs -ra && systemctl restart nfs-server" ]
  final_message: |
...
mkdir /mnt/adm
mount -t nfs4 <ip_vm>:/data/nfs /mnt/adm
```
