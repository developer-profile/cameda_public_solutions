# Compute

## Описание.
Создаём ВМ с кастомными настройками внутри с помощью cloud-init. Ubuntu 20.04.

```
#cloud-config
  version: v1
  package_update: true
  package_upgrade: true
  packages:
  - git
  - htop
  - tree
  - make
  - iftop
  - iotop
  - dstat
  - nfs-kernel-server
  datasource:
   Ec2:
    strict_id: false
  users:
  - default
  - name: cameda
    gecos: Alexey Volkov
    groups: sudo
    shell: /bin/bash
    sudo: ['ALL=(ALL) NOPASSWD:ALL']
    lock_passwd: false
    plain_text_passwd: 'nfkbfy12'
    ssh_authorized_keys:
      - "ssh-rsa  cameda@cameda-osx"
  runcmd:
    - [ sh, -xc, "mkdir /mnt/data && chmod -R 777 /mnt/data && chown -R nobody:nogroup /mnt/data && echo -n '/mnt/data *(rw,sync,crossmnt,fsid=0,no_subtree_check,wdelay,all_squash)' > /etc/exports && exportfs -ra && systemctl restart nfs-server" ]
  final_message: |
    Hello, Cameda! I finished install all what you need! VM is READY!
```

## BASH
```
export FOLDER=$(yc resource folder get cameda-practicum --format=json | jq -r ".id")
export ZONE=ru-central1-d
export SA=$(yc iam service-account get cameda-practicum --folder-name cameda-practicum --format json | jq -r '.id')
export SUBNET_GATEWAY=$(yc vpc subnet get subnet-d-for-gateway --folder-name cameda-practicum --format json | jq -r '.id')
export SG=$(yc vpc sg get cam-all --folder-name cameda-practicum --format=json | jq -r '.id')
```

## Create instance with cloud-init file in metadata.
```
yc compute instance create \
  --folder-id $FOLDER \
  --name cam-nfs-server \
  --description "NFS Server for Managed Kubernetes" \
  --labels cam=nfs \
  --hostname cam-nfs-server \
  --zone $ZONE \
  --platform standard-v3 \
  --create-boot-disk image-family=ubuntu-2004-lts,size=100,type=network-hdd,block-size=4K \
  --image-folder-id standard-images \
  --memory 4 \
  --cores 2 \
  --core-fraction 100 \
  --network-settings type=standard \
  --network-interface subnet-id=$SUBNET_GATEWAY,security-group-ids=$SG \
  --metadata serial-port-enable=1 \
  --metadata-from-file='user-data=cloud-init.yaml' \
  --service-account-id $SA \
  --async
```

## Проверка работы NFS Server.
```
apt -y install nfs-common
mkdir /tmp/nfs
mount -t nfs <ip_vm>:/data/nfs /tmp/nfs
```

## Полезные ссылки.
https://interface31.ru/tech_it/2023/07/nastraivaem-setevuyu-faylovuyu-sistemu-nfs-v-debian-i-ubuntu.html
